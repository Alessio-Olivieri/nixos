#!/usr/bin/env python3
"""One normal logout request, bound to the original session, never forced."""
import json
import os
...
try:
    fields = Path('/proc/77549/stat').read_text().rsplit(')', 1)[1].split()
except FileNotFoundError:
    raise SystemExit('Original session already ended; refusing another logout')
if fields[0] == 'Z' or fields[19] != '811511':
    raise SystemExit('Original session changed; refusing another logout')
state = json.loads(subprocess.check_output([
    '/nix/store/vd22jn7vwc1ha92msyh4h3d8fj7328j0-precision-scanout-session-test/bin/precision-scanout-session-control',
    'status'], timeout=5))
